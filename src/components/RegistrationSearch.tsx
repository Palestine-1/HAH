...
export const RegistrationSearch: React.FC<RegistrationSearchProps> = ({ isDarkMode = false }) => {
  ...
  const searchInSampleData = (searchWords: string[]): Reciter | null => {
    const searchResults: { result: Reciter; score: number }[] = [];
    
    for (const reciter of sampleReciters) {
      const normalizedReciterName = normalizeText(reciter.name);
      const resultWords = normalizedReciterName.split(' ').filter(word => word.length > 0);
      let score = 0;
      ...
      searchWords.forEach(word => {
        ...
        else if (normalizedReciterName.includes(word)) {
          score += 20;
        }
      });
      ...
      if (score > 0) {
        searchResults.push({ result: reciter, score });
      }
    }
    ...
    if (searchResults.length > 0) {
      ...
      console.log('Registration search results with scores:', searchResults.map(r => ({
        name: r.result.name,
        score: r.score
      })));
      
      // التحقق من أن أفضل نتيجة لها نقاط كافية لتكون مطابقة جيدة
      if (searchResults[0].score >= 50) {
      ...
      if (error) {
        ...
        if (data && data.length > 0) {
          ...
          if (matchResults.length > 0) {
            ...
            // طباعة النتائج للتطوير (يمكن إزالتها لاحقاً)
            console.log('Database registration search results with scores:', matchResults.map(r => ({
              name: r.result.name,
              score: r.score
            })));
            
            // التحقق من أن أفضل نتيجة لها نقاط كافية لتكون مطابقة جيدة
            if (matchResults[0].score >= 50) {
