...
import { Search, User, AlertTriangle, Database } from "lucide-react";
...
export default function SearchSection({
  ...
  const sampleResults: Result[] = [
    { id: 1, name: "أحمد محمد علي", category: "3", grade: 95, rank: 1 },
    { id: 2, name: "فاطمة إبراهيم حسن", category: "3", grade: 92, rank: 2 },
    { id: 3, name: "عمر حسن محمود", category: "3", grade: 88, rank: 3 },
    { id: 4, name: "نور الدين أحمد", category: "5", grade: 96, rank: 1 },
    { id: 5, name: "مريم سالم يوسف", category: "5", grade: 91, rank: 2 },
    { id: 6, name: "حسام النجار", category: "8", grade: 94, rank: 1 },
    { id: 7, name: "ليلى الخطيب", category: "8", grade: 89, rank: 2 },
    { id: 8, name: "يوسف الأحمد", category: "10", grade: 97, rank: 1 },
    { id: 9, name: "زينب محمد", category: "15", grade: 93, rank: 1 },
    { id: 10, name: "خالد عبدالله", category: "20", grade: 90, rank: 1 },
    { id: 11, name: "آية حسام", category: "25", grade: 95, rank: 1 },
    { id: 12, name: "محمد الطيب", category: "30", grade: 98, rank: 1 },
  ];
  ...
  const calculateRankInCategory = async (
    studentGrade: number,
    category: string
  ) => {
    try {
      ...
      if (!isConnected) {
        // حساب الترتيب من البيانات التجريبية
        const categoryResults = sampleResults.filter(
          (r) => r.category === category
        );
        const betterGrades = categoryResults.filter(
          (r) => r.grade > studentGrade
        );
        return betterGrades.length + 1;
      }
  ...
  const handleSearch = async () => {
    ...
    if (
      ...
      setSearchError(
        "يجب كتابة الاسم الأول والثاني على الأقل أو اسم واحد بـ 3 أحرف على الأقل"
      );
      onSearch(null);
      return;
    }
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        console.log("No results found in database, trying sample data");
        // البحث في البيانات التجريبية
        const sampleResult = searchInSampleData(searchWords);
        onSearch(sampleResult);
      }
      ...
      console.error("Search error:", error);
      setSearchError("حدث خطأ أثناء البحث - جاري البحث في البيانات المحلية");
      setConnectionError(true);
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="flex justify-center items-center gap-3 mb-4">
              ...
              <h2
                className={`text-4xl md:text-5xl font-bold ${
                  isDarkMode ? "text-gray-100" : "gradient-text-animated"
                }`}
              >
                البحث عن النتائج
              </h2>
          ...
          {connectionError && (
            ...
            >
              ...
              <p className="text-sm">
                يتم البحث في البيانات المحفوظة محلياً - النتائج الكاملة ستظهر
                عند توفر الاتصال
              </p>
            </div>
          )}
          ...
          {/* Search Section */}
          <div
            className={`p-8 rounded-3xl shadow-2xl mb-8 transition-colors duration-300 ${
              isDarkMode
          ...
          >
            <div className="flex flex-col sm:flex-row gap-4 mb-6">
              <div className="flex-1">
                <div
                  className={`flex rounded-2xl overflow-hidden border-2 focus-within:border-blue-500 transition-all duration-300 shadow-lg ${
                    isDarkMode
            ...
            {searchError && (
              <div
                className={`p-4 rounded-xl border-2 text-center animate-fadeIn ${
                  isDarkMode
